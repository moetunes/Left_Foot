...
void read_rcfile() {
    ...
    if ( rcfile == NULL ) {
        fprintf(stderr, "\033[0;34m:: Left_Foot : \033[0;31m Couldn't find %s\033[0m \n" ,RC_FILE);
        set_defaults();
        return;
    } else {
...
void get_font() {
	...
	if(missing) {
		if(FONTS_ERROR < 1)
            while(--n)
                fprintf(stderr, ":: Left_Foot :: missing fontset: %s\n", missing[n]);
	...
	if(font.fontset) {
		...
		fprintf(stderr, ":: Left_Foot :: Font '%s' Not Found\n:: Left_Foot :: Trying Font 'Fixed'\n", font_list);
		if(!(font.font = XLoadQueryFont(dis, font_list))
		&& !(font.font = XLoadQueryFont(dis, "fixed")))
			fprintf(stderr, ":: Left_Foot :: Error, cannot load font: '%s'\n", font_list);
